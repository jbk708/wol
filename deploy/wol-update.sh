#!/usr/bin/env bash
# Pull the newest ghcr.io/lucaspatel/wol image and recreate the container only if it changed.
...
if [[ "$before" != "$after" ]]; then
    ...
    # Old :wol3 images are left dangling by the pull; keep a week of them for quick rollback.
    docker image prune -f \
        --filter "label=org.opencontainers.image.source=https://github.com/lucaspatel/wol" \
